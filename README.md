...
It began as an integration of two Operating Systems lab assignments
(a FAT filesystem/shell lab and a process-scheduling lab) reorganised
into one coherent, testable project. See `docs/architecture.md` and
`docs/design.md` for the full design rationale and how it maps back to
the original labs.
...
- Single-threaded, single-user: there is no concurrent access to the
  virtual disk from multiple shells/processes (intentionally — see
  `docs/architecture.md` for why shared memory was dropped from the
  LA10 design).
