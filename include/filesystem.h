 ...
 * heap buffer, not a SysV shared-memory segment as in the LA10 reference
 * lab). MiniOS is one program embedding filesystem + process manager +
 * scheduler + shell, so there is no need for multiple processes to share
 * the disk image; using shared memory here would only add complexity
 * without benefit. See docs/design.md for the full rationale.
 */
