...
divided into `65536` blocks of `1024` bytes each — the same block size
and block count as the LA10 reference lab, which sized its bitmap and
FAT regions to exactly cover a 64 MB disk.
...
`size` field doubles as **the directory's live entry count** — this
mirrors the LA10 convention and avoids needing a separate "directory
header" struct. Entry `1` is always `".."`, pointing at the parent's
...
priority. This intentionally drops LA5's real multi-process
launcher/timer/signal machinery (`SIGUSR1`-driven preemption across
real Unix processes synchronized via semaphores) because that
mechanism exists to demonstrate *real* OS-level context switching
across `fork()`ed processes — which is explicitly out of scope here (no
ELF loading, no real preemption). What MiniOS keeps from LA5 is the
part that is actually the subject of this project: the PCB fields,
process states, and — most importantly — the **scheduling algorithm**.
...
Default quanta are `{10, 5, 2}` for priorities `{0, 1, 2}` — taken
directly from LA5's `quantum[3] = {10, 5, 2}` — but `sched_init()`
accepts a custom 3-element array so this is configurable, as required.
...
- **In-process heap buffer instead of SysV shared memory.** See
  `docs/architecture.md` for the rationale; this is the most visible
  deviation from the LA10 reference and is called out explicitly per
  the project's "explain significant deviations" rule.
- **No separate `HD2VD.c`/`foosh.c`-style multi-binary split.** LA10
  split disk creation (`diskmanager`), the shell (`foosh`), and shared
  helpers (`diskutils`) into separate compiled programs communicating
  via shared memory. Since MiniOS is one process, that split collapses
  naturally into `filesystem.c` (disk + diskutils' logic) plus
  `shell.c` (foosh's command loop), matching the project's required
  file layout.
